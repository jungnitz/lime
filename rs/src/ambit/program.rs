...
impl<'a> ProgramState<'a> {
    ...
    pub fn compute(
        ...
        let (matches, _) = self.get_mapping(&mut signals, operands);
        self.optimize_dcc_usage(&mut signals, operands, &matches);
        ...
        for i in 0..3 {
            ...
            self.signal_copy(signals[i], SingleRowAddress::Bitwise(operands[i]), free_dcc);
        }

        // all signals are in place, now we can perform the MAJ operation
        self.maj(maj_id, signal, out_address);
    }
    ...
    fn optimize_dcc_usage(
        ...
        while changed {
            ...
            for i in 0..3 {
                if matching[i]
                    || operands[i].is_dcc()
                    || self.rows().get_rows(signals[i]).next().is_some()
                {
                    continue;
                }
    ...
    fn get_mapping(
        ...
        let operand_signals = operands.map(|op| self.rows().get_operand_signal(op));
    ...
    pub fn snapshot(&mut self) {
        self.rows.snapshot();
        self.snapshot_len = self.instructions.len();
    }

    pub fn rollback(&mut self) {
        self.rows.rollback();
        let sn = self.snapshot_len;
        self.instructions.truncate(sn);
    }
